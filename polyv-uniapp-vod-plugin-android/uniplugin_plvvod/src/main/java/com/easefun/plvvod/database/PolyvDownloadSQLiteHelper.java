...
public class PolyvDownloadSQLiteHelper extends SQLiteOpenHelper {
     ...
     * @param vid
     */
    public void delete(final String vid) {
        SINGLE_EXECUTOR.execute(new Runnable() {
            @Override
            public void run() {
                SQLiteDatabase db = getWritableDatabase();
    ...
    public boolean isAdd(String vid) {
        SQLiteDatabase db = getWritableDatabase();
        String sql = "select vid ,duration,filesize,bitrate,fileType from downloadlist where vid=? ";
        Cursor cursor = null;
        try {
    ...
    /**
     * 根据vid+bitrate+filetype条件，判断该下载信息是否已以添加过
     *
     * @param info
     * @return
     */
     ...
     * @return
     */
    public PolyvDownloadInfo getDownloadInfo(String vid) {
        SQLiteDatabase db = getWritableDatabase();
        String sql = "select vid ,bitrate,percent from downloadlist where vid=? ";
        Cursor cursor = null;
        try {
            cursor = db.rawQuery(sql, new String[]{vid});
            if(cursor.moveToNext()){
                ...
                PolyvDownloadInfo info = new PolyvDownloadInfo(vid, bitrate);
